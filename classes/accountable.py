...
class Accountable:
    ...
    def get_next_due_date(self, start_date=date.today()):
        if self.interval == Interval.Once:
            ...
            next_due_days = 7 - (days_since_start % 7)
            return start_date + timedelta(days=next_due_days)
            ...
            return date(year_adjustment, next_month, self.start_date.day)
        elif self.interval == Interval.Yearly:
            if start_date >= date(start_date.year, self.start_date.month, self.start_date.day):
                return date(start_date.year + 1, self.start_date.month, self.start_date.day)
            else:
                return date(start_date.year, self.start_date.month, self.start_date.day)
        else:
